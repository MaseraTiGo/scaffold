...
class AlipayMiniExtend(object):
    ...
    def get_appid(self):
        return "2019102268569102"

    def get_payee_user_id(self):
        return "2088631636107636"

    def _get_category(self):
        return "RENT_DIGITAL"

    def _get_notify_url(self, method):
        notify_url = ""
        if method == "alipay.fund.auth.order.app.freeze":
            notify_url = "http://crm-test.dsggy.cn/interface/alipay_freeze_notify"
        return notify_url
    ...
    def _pack_capital(self, method, appid, **param):
        ...
        base_param.update({"method":method, \
                           "biz_content":json.dumps(param, separators = (',', ':'))})
        notify_url = self._get_notify_url(method)
        if notify_url:
            base_param.update({"notify_url":notify_url})
    ...
    def alipay_freeze_query(self, **dic_param):
        ...
        params = {"auth_no":dic_param["auth_no"], "operation_id":dic_param["operation_id"]}
    ...
    def alipay_freeze(self, **dic_param):
        ...
        params = {"out_order_no":"{order_sn}{time_mark}".format(order_sn = dic_param["order_no"], time_mark = str(int(time.time() * 1000))), \
                  "out_request_no":self._get_out_request_no(), \
                  "order_title":"授权冻结", "amount":dic_param["amount"] / 100, "product_code":"PRE_AUTH_ONLINE", \
                  "payee_user_id":self.get_payee_user_id(), "extra_param":{"category":self._get_category()}}
    ...
    def alipay_unfreeze(self, **dic_param):
        ...
        params = {"auth_no":dic_param["auth_no"], "out_request_no":self._get_out_request_no(), \
                  "remark":"{unfreeze_time}解冻{money}".format(unfreeze_time = self._get_timestamp(), money = dic_param["amount"] / 100), \
                  "amount":dic_param["amount"] / 100}
        if dic_param["deposit_type"] == "credit":  # 冻结类型，押金or信用分
            params.update({"extra_param":{"unfreezeBizInfo": "{\"bizComplete\":\"true\"}"}})
        result = self._request_api('have_biz', method, self.get_appid(), **params)
        return result
    ...
    def alipay_freeze_trade(self, **dic_param):
        ...
        params = {"out_trade_no":self._get_out_request_no(), "auth_no":dic_param["auth_no"], \
                  "product_code":"PRE_AUTH_ONLINE", "subject":"预授权转支付", "buyer_id":dic_param["buyer_id"], \
                  "seller_id":dic_param["seller_id"], "total_amount":dic_param["total_amount"] / 100, \
    ...
    def get_chance_qrcode(self, **dic_param):
        ...
        params = {"url_param":dic_param["url_param"], "query_param":query_param, \
