...
class MiniMch(object):
    ...
    @classmethod
    def get_sign(cls, param):
        # 计算签名
        url_str = utils.get_urlparam(param)
        stringSignTemp = url_str + '&key=' + cls.get_key()
        hl = hashlib.md5()
    ...
    @classmethod
    def check_sign(cls, kwargs):
        check_sign = kwargs.pop('sign')
        sign = cls.get_sign(kwargs)
        return bool(check_sign == sign)

    """加密信息解密"""
    @classmethod
    def req_info_decrypt(cls, req_info):
        hl = hashlib.md5()
        hl.update(cls.get_key().encode(encoding = 'utf-8'))
    ...
    """统一下单"""
    def unifiedorder(self, out_trade_no, price, product_id = '', body = '', trade_type = 'NATIVE', openid = ''):
        url = 'https://api.mch.weixin.qq.com/pay/unifiedorder'
        notify_url = self.get_notify_url() + '/interface/pay_notify'
        param = {
            'appid': self.appid,
            'mch_id': self.get_mchid(),
            'nonce_str': utils.get_nonce_str(),
            'device_info': 'WEB',
            'sign_type': 'MD5',
            'body': body,
        ...
        resp = requests.post(url, data = xml_str.encode('utf-8'), headers = {'Content-Type': 'text/xml'})
        data = resp.text.encode('ISO-8859-1').decode('utf-8')
        data = xmltodict.parse(data)['xml']
        return data
        # if data.get('return_code') != 'SUCCESS':
        #     logger.error('统一下单接口调用失败，订单号（{out_trade_no}），{return_msg}'\
        #                  .format(out_trade_no = out_trade_no, return_msg = data['return_msg']))
        #     return False
        # if data.get('result_code') != 'SUCCESS':
        #     logger.error('统一下单接口调用失败，订单号（{out_trade_no}），错误代码：{err_code}，错误描述：{err_code_des}'\
        #                  .format(out_trade_no = out_trade_no, err_code = data['err_code'], err_code_des = data['err_code_des']))
        #     return False
        # return data
    ...
    def refund(self, out_trade_no, out_refund_no, total_fee, refund_fee):
        ...
        notify_url = self.get_notify_url() + '/interface/refund_notify'
        params = {
            "appid": self.appid,
            "mch_id": self.get_mchid(),
            "nonce_str": utils.get_nonce_str(),
