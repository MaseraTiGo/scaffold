...
class SetStatus(StaffAuthorizedApi):
    request = with_metaclass(RequestFieldSet)
    request.goods_id = RequestField(IntField, desc="商品id")
    request.review_info = RequestField(
        DictField,
        desc="商品审核结果",
        conf={
            'remark': CharField(desc="备注", is_required=False),
            'status': CharField(desc="审核状态", choices=ReviewStatus.CHOICES),
        }
    )
    ...
    def execute(self, request):
        ...
        request.review_info.update({'staff_id': staff.id})
        GoodsReviewServer.set_review_result(request.goods_id, **request.review_info)
